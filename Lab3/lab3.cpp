...
void load_net(cv::dnn::Net &net) {
  auto result = cv::dnn::readNetFromDarknet(
      "config_files/yolo" + YOLO_VERSION + ".cfg",
      "config_files/yolo" + YOLO_VERSION + ".weights");
...
int main(int argc, const char *argv[]) {
  ...
  model.detect(image, classIds, confidences, boxes, .2, .4);
  ...
  for (int i = 0; i < detections; ++i) {
    ...
    cv::putText(image, class_list[classId].c_str(), cv::Point(box.x, box.y - 5),
